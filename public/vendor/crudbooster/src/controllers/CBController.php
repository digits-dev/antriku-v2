...
//                                 $relationship_table_pk => CRUDBooster::newId($ro['relationship_table']),
    ...
    public function postActionSelected()
    {
        ...
        if (! $id_selected) {
            CRUDBooster::redirect($_SERVER['HTTP_REFERER'], "Please select a row.", 'warning');
        }
        ...
        if($button_name == 'print_receive_form') 
		{
            if(count($id_selected) > 1){
                return redirect()->back()->with(['message_type' => 'warning', 'message' => 'Select one row only.']);
            }else{
                return redirect(CRUDBooster::mainpath('PrintReceivingForm/'.$id_selected[0]));
            }

        }elseif($button_name == 'print_technical_report')
        {
            if(count($id_selected) > 1){
                return redirect()->back()->with(['message_type' => 'warning', 'message' => 'Select one row only.']);
            }else{
                return redirect(CRUDBooster::mainpath('PrintTechnicalReport/'.$id_selected[0]));
            }
            
        }elseif($button_name == 'print_release_form')
        {
            if(count($id_selected) > 1){
                return redirect()->back()->with(['message_type' => 'warning', 'message' => 'Select one row only.']);
            }else{
                return redirect(CRUDBooster::mainpath('PrintReleaseForm/'.$id_selected[0]));
            }
