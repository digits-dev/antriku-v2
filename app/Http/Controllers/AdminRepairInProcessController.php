<?php namespace App\Http\Controllers;
	...
	class AdminRepairInProcessController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Date Received","name"=>"level4_personnel_edited"];
...
// 			$this->button_selected[] = ['label'=>'Print Release Form', 'icon'=>'fa fa-print', 'name'=>'print_release_form'];
		...
		public function getDetail($id) 
		{
			...
			$data = [];
			$data['page_title'] = "Repair In Process";
	    ...
	    */
	    public function hook_query_index(&$query) {
			...
			if(CRUDBooster::isSuperadmin() || CRUDBooster::myPrivilegeId() == 6){
			    $query->where('repair_status', 4)->orderBy('id', 'asc'); 
			}else{
			    $query->where('repair_status', 4)->where('branch', CRUDBooster::me()->branch_id); 
...
// 			}
	    ...
	    */
		...
		public function DiagnoseTransactionProcess(Request $request)
		{
			...
			return redirect('admin/repair_in_process');
		}
	}