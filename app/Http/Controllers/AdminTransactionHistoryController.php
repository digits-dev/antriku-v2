...
class AdminTransactionHistoryController extends \crocodicstudio\crudbooster\controllers\CBController
{
	...
	public function cbInit()
	{
		...
		if (CRUDBooster::myPrivilegeId() == 10) {
			$this->addaction[] = [
				'title'   => 'VOID',
				'icon'    => 'fa fa-ban',
				'url'     => 'javascript:showVoidAlert([id])', 
				'color'   => 'danger',
				'showIf'  => "[repair_status] != 3",
			];
		}
		...
		$this->post_index_html = "
			...
			<script src=''.asset('js/voidTransaction.js').''></script>
	        "
			;
		...
		$this->load_js[] = asset('js/voidTransaction.js');
	...
	{
		//Your code here
		if (in_array(CRUDBooster::myPrivilegeId(), [3,4,9])) {
			$query->where('branch', CRUDBooster::me()->branch_id)->orderBy('returns_header.updated_at', 'desc');
		}else {
			$query->orderBy('returns_header.updated_at', 'desc');
		}
	}
	...
	public function voidTransaction(Request $request)
	{
		...
		$transaction = DB::table('returns_header')->where('id', $request->id)->first();
