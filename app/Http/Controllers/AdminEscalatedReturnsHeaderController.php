<?php namespace App\Http\Controllers;
	...
	class AdminEscalatedReturnsHeaderController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label" => "Escalated By", "name" => "escalated_by", 'join' => 'cms_users,name'];
			$this->col[] = ["label" => "Escalated At", "name" => "escalated_at"];
	        
	    }
		
		public function hook_query_index(&$query) {
			if (CRUDBooster::myPrivilegeId() == 2) {
				$query->leftJoin('cms_users as escalated_user', 'escalated_user.id', '=', 'returns_header.escalated_by')
					  ->whereIn('repair_status', [23])
					  ->where('branch', CRUDBooster::me()->branch_id)
					  ->where('escalated_user.id_cms_privileges', 3);
			} elseif (CRUDBooster::myPrivilegeId() == 8) {
				$query->leftJoin('cms_users as escalated_user', 'escalated_user.id', '=', 'returns_header.escalated_by')
					  ->whereIn('repair_status', [23])
					  ->where('branch', CRUDBooster::me()->branch_id)
					  ->where('escalated_user.id_cms_privileges', 4);
			}
		}		
