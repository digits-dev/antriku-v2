            ...
            <div class="col-md-12">
                <div class="m-dash-card m-dash-default clickable_card1" id="clickable_card1"
                    ...
                    <div class="m-dash-card-value" id="filtered-callouts-value">
                        {{ count($myOngoingDiagnosis_bgc) ?? 0 }}
                    </div>
                </div>
            </div>
                ...
                <div class="transactions-container">
                    ...
                    <div class="card-body-dash">
                        <div class="chart-container"
                            ...
                            <canvas id="assigningJoChart-bgc" height="200"></canvas>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="transactions-container">
                <div class="transactions-header" style="margin: 0%">
                    ...
                    <div class="search-container">
                        ...
                        <input type="text" class="search-input search-input-bgc-tech"
                    ...
                    <table class="transactions-table" autofocus>
                        ...
                        <tbody class="tech-tbody" style="text-align: left; background: rgba(240, 246, 247, 0.7);">
                            @php
                                ...
                                function mergeTechDataBGC(&$allDataBGC, $collection, $label)
                                {
                                    foreach ($collection as $item) {
                                        $id = $item->id ?? ($item->reference_no ?? uniqid()); // fallback in case no ID

                                        if (!isset($allDataBGC[$id])) {
                                            $item->status_labels = [$label];
                                            $allDataBGC[$id] = $item;
                                        } else {
                                            // If already exists, just add the label
                                            if (!in_array($label, $allDataBGC[$id]->status_labels)) {
                                                $allDataBGC[$id]->status_labels[] = $label;
                                            }
                                        }
                                    }
                                }

                                mergeTechDataBGC($allDataBGC, $myOngoingDiagnosis_bgc, 'diagnosing');
                                mergeTechDataBGC($allDataBGC, $myOngoingRepair_bgc, 'repairing');
                                mergeTechDataBGC($allDataBGC, $myAwaitingRepair_bgc, 'awaiting');

                                mergeTechDataBGC($allDataBGC, $totalCarryIn_bgc, 'carry-in');
                                mergeTechDataBGC($allDataBGC, $totalMailIn_bgc, 'mail-in');
                                mergeTechDataBGC($allDataBGC, $totalIW_bgc, 'in-warranty');
                                mergeTechDataBGC($allDataBGC, $totalOOW_bgc, 'out-of-warranty');

                                mergeTechDataBGC($allDataBGC, $totalToAsign_bgc, 'to-assign-jo');
                                mergeTechDataBGC($allDataBGC, $totalPedningAssigned_bgc, 'pending-assigned-jo');

                                // Just for chart counts
                                $total_carry_in_bgc = $totalCarryIn_bgc->count();
                                $total_mail_in_bgc = $totalMailIn_bgc->count();
                                $total_iw_bgc = $totalIW_bgc->count();
                                $total_oow_bgc = $totalOOW_bgc->count();

                                $total_to_assing_jo_bgc = $totalToAsign_bgc->count();
                                $total_pending_assigned_jo_bgc = $totalPedningAssigned_bgc->count();
                            @endphp

                            @foreach ($allDataBGC as $item)
                ...
                <div class="card-footer-dash">
                    {{-- <span class="card-footer-text-dash showing_data_tech_overview"
                        ...
                        Showing {{ count($allDataBGC) }} Total of Data
                    </span> --}}
                </div>
            </div>
        </div>
        ...
        <div class="col-md-12">
            <div class="transactions-container">
                ...
                <div class="card-body-dash">
                    <div class="chart-container"
                        style="position: relative; height: 330px; width: 100%; display: flex; flex-direction: column; align-items: center; padding: 10px 0;">
                        <canvas id="warrantyChart-bgc" height="200"></canvas>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="transactions-container">
                ...
                <div class="card-body-dash">
                    <div class="chart-container"
                        ...
                        <canvas id="repairTypeChart-bgc" height="200"></canvas>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        // ====================
        const assigningJoCtx = document.getElementById('assigningJoChart-bgc').getContext('2d');
        const totalAssigning = {{ $total_to_assing_jo_bgc ?? 0 }} + {{ $total_pending_assigned_jo_bgc ?? 0 }};
        const assigningChart = new Chart(assigningJoCtx, {
            type: 'doughnut',
            data: {
                labels: ['To Assign JO', 'Pending Assigned JO'],
                datasets: [{
                    data: [{{ $total_to_assing_jo_bgc ?? 0 }}, {{ $total_pending_assigned_jo_bgc ?? 0 }}],
        ...
        // ====================
        const repairTypeCtx = document.getElementById('repairTypeChart-bgc').getContext('2d');
        const total = {{ $total_carry_in_bgc ?? 0 }} + {{ $total_mail_in_bgc ?? 0 }};
        const repairChart = new Chart(repairTypeCtx, {
            type: 'doughnut',
            data: {
                labels: ['Carry-In', 'Mail-In'],
                datasets: [{
                    data: [{{ $total_carry_in_bgc ?? 0 }}, {{ $total_mail_in_bgc ?? 0 }}],
                    backgroundColor: ['#222831', '#948979'],
                    borderWidth: 5,
                    hoverOffset: 10,
                    borderRadius: 8
        ...
        // Bar Chart Setup
        // ====================
        const warrantyCtx = document.getElementById('warrantyChart-bgc').getContext('2d');
        const warrantyChart = new Chart(warrantyCtx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Repair Cases',
                    data: [{{ $total_iw_bgc ?? 0 }}, {{ $total_oow_bgc ?? 0 }}],
...
<script>
    $(document).ready(function() {
        $('.search-input-bgc-tech').on('keyup', function() {
