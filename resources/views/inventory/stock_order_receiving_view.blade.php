                ...
                <div class="col-md-12">
                    <div>
                        <div class="table-container-stock-reserve" style="height: 430px; overflow:auto">
                            <table class="reserved-parts-table">
                                ...
                                <tbody id="reservedPartsTableBody">
                                    @foreach ($stock_order_data as $item)
                                        <tr>
                                            <td>
                                                <input type="checkbox">
                                            </td>
                                            ...
                                            <td>
                                                ...
                                                <input type="number"
                                                        class="input-cus receive-qty"
                                                        max="{{$item->ordered_qty}}"
                                                        min="{{$item->ordered_qty}}"
                                                        data-line-id="{{ $item->lines_id }}"
                                                        data-max="{{ $item->ordered_qty }}">
                                            </td>
                                            <td>
                                                <div class="stock-status">
                                                    <div class="stock-indicator {{$item->order_status == 'Awaiting Apple Stock (Buffer)' ? 'stock-pending' : 'stock-confirmed-deducted'}}"></div>
                                                    {{$item->order_status}}
                                                </div>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <button type="button" onclick="singleReceiving({{$item->lines_id}})" class="btn btn-success">Receive</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <div class="pull-right">
                            <button type="button" class="btn btn-info">Bulk Receiving</button>
                        </div>
                    </div>
                </div>
...
@push('bottom')
    <script>
        // function singleReceiving(buttonElement) {
        ...
        //     // Confirm
        //     if (!confirm(`Receive ${quantity} items?`)) return;

        //     $.ajax({
        //         url: '/receive-stock-line',
        //         method: 'POST',
        //         data: {
        //             line_id: lineId,
        //             quantity: quantity,
        ...
        //                 location.reload(); // or update the row status
        //             } else {
